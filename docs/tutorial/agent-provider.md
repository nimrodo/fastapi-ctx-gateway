...
`my_runnable_or_graph` is duck-typed: anything exposing `.astream`/`.stream`/`.ainvoke`/`.invoke` works, in that order of preference (see [`Provider`][fastapi_ctx_gateway.providers.base.Provider]). It's text-only for v1 — a request containing a binary part is rejected with a neutral error event (`agent_input_unsupported`); see [issue #27](https://github.com/nimrodo/fastapi-ctx-gateway/issues/27) for multimodal support.
