...
| `prompt_injection_detections_total{mode}` | A request's content was flagged as a likely prompt injection — labeled by `prompt_injection_mode` |
| `prompt_injection_fail_open_total` | An injection-detector call failed/timed out and fell open (treated as no match, not a genuine negative) |
...
- **`request.pre_proxy`** — wraps injection detection, the rate-limit check, circuit-breaker check, and pruning on a miss, ending right before the first byte is proxied from the configured provider.
...
The same applies to `prompt_injection_fail_open_total` when a `local_classifier` backend is configured: it climbing means the classifier is degraded (every affected request silently skipped detection), not that traffic is genuinely clean.
