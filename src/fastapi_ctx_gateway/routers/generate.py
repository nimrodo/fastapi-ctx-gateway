...
Thin orchestrator: sequences auth, injection detection, rate-limit,
pruning, cache, and the provider call. Business logic lives in the
...
async def stream_generate_content(
    ...
    injection_detector: InjectionDetector | None = Depends(get_injection_detector),
) -> StreamingResponse:
    """Proxy a streaming generate request through to the named provider.

    Sequenced deliberately: auth -> injection detection -> rate-limit
    check -> breaker precheck -> prune -> cache-eligibility -> cache
    lookup -> (hit: return) / (miss: proxy). A rejected request never
    reaches pruning or cache work, let alone the provider — and the
    breaker check (in-memory, O(1)) happens before any of that, since
    it's the cheapest possible reject. Admission control is checked
    against the client's original, unpruned token estimate — pruning
    happens only after a request has already been admitted, and the
    cache is looked up against the *pruned* turns (better hit rate,
    cheaper to embed).
    ...
    with pre_proxy_span():
        if settings.prompt_injection_mode != "off":
            assert injection_detector is not None  # enforced at boot, see app.py
            if await injection_detector.detect(turns=request.turns, system=request.system):
                metrics.prompt_injection_detections.labels(
                    mode=settings.prompt_injection_mode
                ).inc()
                logger.warning(
