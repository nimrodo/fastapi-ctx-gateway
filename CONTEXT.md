...
**Miss path** — the code path taken when no cache hit occurs (cache disabled, ineligible, or a genuine miss): auth → injection detection → rate-limit → breaker-check → prune, then proxy through to the configured provider. The gateway's own overhead on this path (everything before the first byte is proxied) targets low single-digit ms, additive to the provider's own latency. See `request.pre_proxy` (OTel span).
...
**Fail-open** — the shared failure contract for optional subsystems: any lookup/inference error or timeout is treated as "disabled for this request" (a cache miss, or no injection match), never as a request failure. The semantic cache (any lookup/store error, timeout, or unreachable Redis at startup) and the prompt-injection detector (any classification error or timeout) both follow it — neither may become a hard dependency for serving traffic. Contrast with **fail-closed** (not used anywhere in this gateway). Distinct from **boot-time misconfiguration** of the injection detector (mode enabled with no usable backend), which fails loud rather than open — see ADR-0009.
